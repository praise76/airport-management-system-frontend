...
export interface RosterGridProps {
  ...
  users: any[]; // User type
  onCellClick: (userId: string, date: Date, existingEntry?: RosterEntry) => void;
}
...
export function RosterGrid({ roster, users, onCellClick }: RosterGridProps) {
  ...
  const getShiftColor = (shift: string) => {
    switch (shift) {
      case 'morning': return 'bg-yellow-100 text-yellow-800 border-yellow-200 hover:bg-yellow-200';
      case 'afternoon': return 'bg-orange-100 text-orange-800 border-orange-200 hover:bg-orange-200';
      case 'night': return 'bg-indigo-100 text-indigo-800 border-indigo-200 hover:bg-indigo-200';
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr>
            ...
            {dates.map((date) => (
              <th key={date.toString()} className="p-2 border-b bg-muted/50 min-w-[120px] text-center font-medium">
                <div>{format(date, "EEE")}</div>
                <div className="text-muted-foreground text-xs font-normal">{format(date, "d MMM")}</div>
              </th>
            ))}
          </tr>
        </thead>
              ...
              <td className="p-3 border-b border-r sticky left-0 z-10 bg-card group-hover:bg-muted/5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">{user.position?.name || "Staff"}</div>
                  </div>
                </div>
              </td>
              {dates.map((date) => {
                  ...
                  >
                    {entry ? (
                      <div className={cn(
                        ...
                        <span className="font-semibold uppercase">{entry.shift}</span>
                        <span className="opacity-75">{entry.shiftStartTime?.slice(0, 5)} - {entry.shiftEndTime?.slice(0, 5)}</span>
                      </div>
                    ) : (
                      <div className="h-full w-full flex items-center justify-center opacity-0 group-hover:opacity-100 hover:opacity-60!">
                        <span className="text-2xl text-muted-foreground/20 font-thin">+</span>
                      </div>
                    )}
                  </td>
                );
