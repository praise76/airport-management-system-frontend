...
export const Route = createFileRoute("/stakeholder/dashboard")({
      ...
      quickActions: [
        {
          ...
          href: "/stakeholder/flights",
          icon: <Plane className="h-4 w-4" />,
        },
        {
          label: "Permits",
          href: "/stakeholder/permits",
          icon: <Shield className="h-4 w-4" />,
        },
        {
          label: "Activities",
          href: "/stakeholder/activities",
...
function StakeholderDashboardPage() {
  ...
  const { data: activitiesData } = useStakeholderActivities(orgId || "", {
    limit: 5,
  });
  const { data: permitsData } = useStakeholderPermits(orgId || "", {
    limit: 5,
  });
  const { data: invoicesData } = useStakeholderInvoices(orgId || "", {
    limit: 5,
  });
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <header className="bg-white/5 backdrop-blur border-b border-white/10 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div
              className={`w-12 h-12 bg-linear-to-br ${config.gradient} rounded-xl flex items-center justify-center text-white`}
            >
              {config.icon}
            </div>
            <div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        {/* Welcome Section */}
        <div
          className={`bg-linear-to-r ${config.gradient} rounded-2xl p-6 text-white relative overflow-hidden`}
        >
          <div className="absolute inset-0 bg-black/10" />
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold mb-1">
                Welcome back, {org.contactPerson.split(" ")[0]}!
              </h2>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white/5 backdrop-blur border border-white/10 rounded-xl p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-slate-400 text-sm">Pending Activities</p>
          ...
          <div className="bg-white/5 backdrop-blur border border-white/10 rounded-xl p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-slate-400 text-sm">Active Permits</p>
                <p className="text-2xl font-bold text-white">
                  {stats.activePermits}
                </p>
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {config.quickActions.map((action) => (
              <Link key={action.href} to={action.href}>
                <Button
                  variant="outline"
                  className="w-full h-auto py-4 flex-col gap-2 border-slate-700 hover:bg-white/10 hover:border-slate-600 text-slate-300"
                >
                  {action.icon}
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white/5 backdrop-blur border border-white/10 rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/stakeholder/activities">
                <Button
                  variant="ghost"
                  size="sm"
                  className="text-slate-400 hover:text-white"
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 bg-slate-800/50 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-white font-semibold">
                    {org.performanceRating}/5
                  </span>
                </div>
                <Progress
                  value={(org.performanceRating || 0) * 20}
                  className="h-2"
                />
              </div>
              <div className="p-3 bg-slate-800/50 rounded-lg">
              ...
              <div className="p-3 bg-slate-800/50 rounded-lg">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`font-semibold ${org.outstandingBalance > 0 ? "text-red-400" : "text-emerald-400"}`}
                  >
