...
import { useGetRosters, useGetRoster, useApproveRoster, useDeleteRoster } from "@/features/roster/api";
...
function RosterPlannerPage() {
  ...
  const [editingEntry, setEditingEntry] = useState<RosterEntry | undefined>(undefined);
  ...
  const { data: selectedRoster, isLoading: rosterLoading } = useGetRoster(selectedRosterId || "");
      ...
      <div className="space-y-6 h-full flex flex-col">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
             <Button variant="ghost" size="icon" onClick={() => setSelectedRosterId(null)}>
                <ArrowLeft className="h-4 w-4" />
             </Button>
             <div>
                <h1 className="text-2xl font-bold">{selectedRoster.name}</h1>
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                   <span>{format(parseISO(selectedRoster.startDate), "MMM d")} - {format(parseISO(selectedRoster.endDate), "MMM d, yyyy")}</span>
          ...
          <div className="flex gap-2">
             {selectedRoster.status === 'draft' && (
                 <Button onClick={handleApprove} disabled={approveMutation.isPending} className="gap-2">
        ...
        <div className="flex-1 min-h-0 overflow-auto">
           {rosterLoading ? <p>Loading details...</p> : (
               <RosterGrid 
    ...
    <div className="space-y-6">
       <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Manage duty rosters and shift assignments</p>
          </div>
          ...
          {rosters?.map((roster) => (
             ...
             >
                ...
                <p className="text-sm text-muted-foreground mb-4">
                    {format(parseISO(roster.startDate), "MMM d")} - {format(parseISO(roster.endDate), "MMM d, yyyy")}
                </p>
       ...
       <CreateRosterModal open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen} />
    </div>
  );
}
