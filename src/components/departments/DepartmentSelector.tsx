...
interface DepartmentSelectorProps {
  ...
  multiple?: boolean;
  /**
   * Current selected values (for multiple selection)
   */
  selectedValues?: string[];
  /**
   * Callback for multiple selection
   */
  onMultipleChange?: (departmentIds: string[]) => void;
}
...
export function DepartmentSelector({
  ...
  const handleSelect = useCallback(
    (departmentId: string) => {
      if (multiple && onMultipleChange) {
        const next = selectedValues.includes(departmentId)
          ? selectedValues.filter((id) => id !== departmentId)
    ...
    [multiple, onMultipleChange, selectedValues, onChange],
  );

  const handleRemove = useCallback(
    (departmentId: string, e: React.MouseEvent) => {
      e.stopPropagation();
      if (multiple && onMultipleChange) {
        onMultipleChange(selectedValues.filter((id) => id !== departmentId));
      }
    },
    [multiple, onMultipleChange, selectedValues],
  );
  ...
  if (!multiple && (!tree || flattenedOptions.length < 20)) {
    return (
      <div className={cn("relative", className)}>
        <Select
          value={value || "unselected"}
          onValueChange={(val) =>
            onChange?.(val === "clear" || val === "unselected" ? null : val)
          }
  ...
  // For larger trees or multiple selection, use a custom tree dropdown
  return (
    <div className={cn("relative", className)}>
      <Button
        type="button"
        ...
        disabled={disabled || isLoading}
        className={cn(
          "w-full h-auto min-h-9 py-1.5 px-3 justify-between font-normal",
          error && "border-destructive",
          !value &&
          ...
          {multiple ? (
            selectedValues.length > 0 ? (
              selectedValues.map((id) => {
                const dept = flattenedOptions.find((opt) => opt.id === id);
                if (!dept) return null;
                return (
                  <div
                    key={id}
                    className="flex items-center gap-1 bg-secondary text-secondary-foreground px-1.5 py-0.5 rounded-sm text-xs"
                  >
                    <DepartmentLevelIndicator
                      level={dept.level}
                      className="h-3 w-3"
